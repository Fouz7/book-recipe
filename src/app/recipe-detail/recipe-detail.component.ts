...
export class RecipeDetailComponent {
  ...
  ngOnInit(): void {
    ...
    if (id !== null) {
      ...
      this.recipeBookService.getRecipeDetail(id).subscribe((detail) => {
        ...
        this.starState = this.recipeDetail?.data?.isFavorite
          ? 'star'
          : 'star_border';
      });
    }
  ...
  addFavorite() {
    ...
    if (this.userId !== null && this.recipeId !== undefined) {
      ...
      this.recipeBookService.addFavorite(this.recipeId, userId).subscribe(
        (response) => {
  ...
  removeFavorite() {
    this.confirmationService.confirm({
      message:
        'Are you sure that you want to remove this recipe from favorites?',
      accept: () => {
        if (this.userId === null || this.recipeId === undefined) return;
        this.recipeBookService
          .addFavorite(this.recipeId, this.userId)
          .subscribe((response) => {
            if (this.recipeDetail && this.recipeDetail.data) {
